	...
	async run(ctx: CachedGuildContext<ChatInputCommandInteraction<"cached">>) {
		if (ctx.args.getSubcommand() === "channel") {
			...
			if (!channel || !channel.isSendable()) {
				return ctx.reply(
					"The specified channel is not valid or I don't have permission to send messages there."
				);
			}
			...
			for (const userId of users) {
				...
				const availabilityString = days
					.map((day, index) => (userAvailabilities.includes(index) ? "✅" : "❌"))
					.join(" ");
